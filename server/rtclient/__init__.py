...
class RTClient:
    ...
    async def _receive_message(self):
        async for message in self._client:
            # print(f"_receive_message {message}")
            return message
        return None
    ...
    async def send_audio(self, audio: str) -> None:
        ...
        await self._client.send(InputAudioBufferAppendMessage(audio=audio))
    ...
    async def generate_response(self, response=None) -> RTResponse:
        print(f"generate_response start")
        await self._client.send(ResponseCreateMessage(response={"modalities": ["text", "audio"]}))
        # message = await self._message_queue.receive(lambda m: m.type == "response.created")
        # if message.type == "error":
        #     raise RealtimeException(message.error)
        # assert message.type == "response.created"
        # print(f"generate_response response message: {message}")
    ...
    async def generate_text_response(self):
        print(f"generate_text_response start")
        await self._client.send(ResponseCreateMessage(response={"modalities": ["text"]}))
    ...
    async def events(self) -> AsyncGenerator[RTInputAudioItem | RTResponse]:
        ...
        while True:
            ...
            # print(f"RTClient message in Event: {message}")
            if message is None:
                break
